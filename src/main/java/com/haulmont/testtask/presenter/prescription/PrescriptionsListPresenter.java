...
public class PrescriptionsListPresenter implements PrescriptionsListView.PrescriptionsListViewListener {
    private PrescriptionDao model;
    private PrescriptionsListViewImpl view;

    public PrescriptionsListPresenter(PrescriptionDao model, PrescriptionsListViewImpl view) {
        this.model = model;
        this.view = view;
    ...
    public void buttonClick(PrescriptionListButtons nameOfButton) {
        switch (nameOfButton) {
            case ADD:
                MainUI.getCurrent().addWindow(new PrescriptionWindow(view, CreateUpdate.CREATE));
                break;
            case UPDATE:
                MainUI.getCurrent().addWindow(new PrescriptionWindow(view, CreateUpdate.UPDATE));
                break;
            case REMOVE:
