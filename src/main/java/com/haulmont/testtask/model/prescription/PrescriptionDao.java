...
public class PrescriptionDao implements Dao<Prescription> {
    ...
    public Prescription get(Long id) {
        ...
        try (PreparedStatement statement = ConnectionUtil.getConnection().prepareStatement(sql)) {
            ...
            prescription.setPatient(patientDao.get(resultSet.getLong("patient_id")));
            prescription.setDoctor(doctorDao.get(resultSet.getLong("doctor_id")));
            prescription.setDateCreated(resultSet.getDate("date_created"));
            prescription.setExpirationDate(resultSet.getDate("expiration_date"));
            prescription.setPriority(PrescriptionPriority.valueOf(resultSet.getString("priority")));
    ...
    public void save(Prescription prescription) {
        String sql = "INSERT INTO prescription(description, patient_id, doctor_id, " +
                "date_created, expiration_date, priority)"
                + "VALUES(?, ?, ?, ?, ?, ?)";
    ...
    @Override
    public void update(Prescription oldPrescription, Prescription updatedPrescription) {
        String sql = "UPDATE prescription set description = ?, patient_id = ?, doctor_id = ?, " +
                "date_created = ?, expiration_date = ?, priority = ? " +
                "where id = ?";
        try (PreparedStatement statement = ConnectionUtil.getConnection().prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            statement.setString(1, updatedPrescription.getDescription());
            statement.setLong(2, updatedPrescription.getPatient().getId());
            statement.setLong(3, updatedPrescription.getDoctor().getId());
            statement.setDate(4, updatedPrescription.getDateCreated());
            statement.setDate(5, updatedPrescription.getExpirationDate());
            statement.setString(6, updatedPrescription.getPriority().toString());
    ...
    public Map<Doctor, Long> getQuantityOfPrescriptions() {
        ...
        try (PreparedStatement statement = ConnectionUtil.getConnection().prepareStatement(sql)) {
            ...
            while (resultSet.next()) {
                DoctorDao doctorDao = new DoctorDao();
                Doctor doctor = doctorDao.get(resultSet.getLong("doctor_id"));
                Long numOfPrescriptions = resultSet.getLong("prescriptions_num");
